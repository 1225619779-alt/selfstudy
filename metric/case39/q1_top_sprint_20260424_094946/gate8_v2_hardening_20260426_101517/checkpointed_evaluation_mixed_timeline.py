...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--resume_from_partial", type=str, default="")
...
def main() -> int:
    ...
    print("resume_from_partial:", args.resume_from_partial or "NONE")
    ...
    if args.resume_from_partial:
        resume_path = Path(args.resume_from_partial)
        resume_payload = np.load(resume_path, allow_pickle=True).item()
        resume_steps = int(resume_payload.get("completed_steps", 0))
        if resume_steps <= 0:
            raise RuntimeError(f"Resume file has no completed steps: {resume_path}")
        if int(resume_payload.get("total_steps_requested", total_steps)) != total_steps:
            raise RuntimeError("Resume total_steps_requested does not match requested schedule length.")
        if int(resume_payload.get("start_offset", args.start_offset)) != int(args.start_offset):
            raise RuntimeError("Resume start_offset does not match current request.")
        for key, target in [
            ("timeline_step", timeline_step),
            ("segment_id", segment_id),
            ("idx_summary", idx_summary),
            ...
            target.extend(list(resume_payload.get(key, [])))
        segment_meta.extend(list(resume_payload.get("schedule_segments", [])))
        current_step = resume_steps
        cum_time_total = float(resume_payload.get("summary", {}).get("final_cumulative_stage_time", 0.0))
        cum_cost_total = float(resume_payload.get("summary", {}).get("final_cumulative_delta_cost", 0.0))
        print(f"Resuming from {resume_path} at completed_steps={resume_steps}")

    iterator = iter_with_offset(test_dataloader_unscaled, int(args.start_offset) + int(resume_steps))
    ...
    progress = tqdm(total=total_steps, initial=current_step)
    planned_seg_start = 0
    for seg_id, seg in enumerate(schedule):
        seg_start = planned_seg_start
        seg_end_planned = planned_seg_start + int(seg.length) - 1
        for local_i in range(seg.length):
            planned_step = planned_seg_start + int(local_i)
            if planned_step < resume_steps:
                continue
            step_wall = time.time()
            try:
        ...
        if current_step > seg_start and not any(int(x.get("segment_id", -1)) == seg_id for x in segment_meta):
            seg_end = min(current_step - 1, seg_end_planned)
            segment_meta.append(
                {
                    "segment_id": seg_id,
                    "kind": seg.kind,
        ...
        planned_seg_start += int(seg.length)
