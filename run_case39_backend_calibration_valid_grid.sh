...
if [[ ! -f "$SUMMARY_PATH" ]]; then
  echo "[grid] missing tau summary: $SUMMARY_PATH" >&2
  exit 1
fi
if [[ ! -f "$CLEAN_VALID_CACHE" || ! -f "$ATTACK_VALID_CACHE" ]]; then
  echo "[grid] missing validation caches" >&2
  exit 1
fi
...
exec >>"logs/case39_backend_calibration_valid_grid.log" 2>&1

echo "[grid] invoked_at=$(date -Is)"
echo "[grid] tau_main=$TAU_MAIN"
echo "[grid] tau_strict=$TAU_STRICT"
echo "[grid] mtd_verbose=$DDET_MTD_VERBOSE"
echo "[grid] mtd_is_worst=$DDET_MTD_IS_WORST"
  ...
  for VARRHO in 0.015 0.02 0.03; do
    for UPPER_SCALE in 1.00 1.05 1.10; do
      ...
      if [[ -f "$SUMMARY_OUT" ]]; then
        echo "[grid] skip existing $TAG"
        continue
      fi

      echo "[grid] running $TAG"
