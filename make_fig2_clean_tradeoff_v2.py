...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate Fig. 2 (clean trade-off summary) for the v2 paper worldline."
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="metric/case14/plots_paper_v2",
        help="Directory to save the new Fig. 2 files.",
    )
...
def main() -> None:
    ...
    # Frozen v2 clean results from the current rerun summaries.
    rows = [
        {
            "label": "Baseline\n(tau=-1.0)",
    ...
    plt.title("Clean trade-off summary (v2)")
    plt.grid(True, alpha=0.3)
    plt.legend(fontsize=8)
    plt.tight_layout()

    png_path = out_dir / "fig2_clean_tradeoff_summary_v2.png"
    pdf_path = out_dir / "fig2_clean_tradeoff_summary_v2.pdf"
    ...
    summary = {
        "rows": rows,
        "reductions_pct": reductions_for_json,
        "recommended_use": "Main text Fig. 2 for the v2 worldline. Use Main OP as the primary point; Sensitivity is optional.",
    }
    with open(out_dir / "fig2_clean_tradeoff_summary_v2.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)

    print("Saved:")
    print(png_path)
    print(pdf_path)
    print(out_dir / "fig2_clean_tradeoff_summary_v2.json")
