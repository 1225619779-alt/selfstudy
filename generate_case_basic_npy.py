...
def main() -> None:
    ...
    data_seed = int(os.getenv('DDET_DATA_SEED', '20260417'))
    ...
    print(f'[seed] DDET_DATA_SEED={data_seed}')
    print(f'[1/6] Reading raw CSV files for {sys_config["case_name"]}...')
    ...
    raw_load = raw_load[
        (raw_load['DateTime'] >= '2012-06-01 00:00:00')
        & (raw_load['DateTime'] <= '2012-09-30 23:45:00')
    ].reset_index(drop=True)
    raw_pv = raw_pv[
        (raw_pv['DateTime'] >= '2019-06-01 00:00:00')
        & (raw_pv['DateTime'] <= '2019-09-30 23:45:00')
    ].reset_index(drop=True)
    ...
    load_high, pv_high = improve_resolution(
        load_raw=raw_load.copy(),
        pv_raw=raw_pv.copy(),
        res=sys_config['load_resolution'],
    )

    load_high_to_save = load_high.reset_index().rename(
        columns={load_high.reset_index().columns[0]: 'DateTime'}
    )
    pv_high_to_save = pv_high.reset_index().rename(
        columns={pv_high.reset_index().columns[0]: 'DateTime'}
    )
    ...
    pv_high_cloud_to_save = pv_high_cloud.reset_index().rename(
        columns={pv_high_cloud.reset_index().columns[0]: 'DateTime'}
    )
    ...
    pv_active, pv_reactive = gen_pv(
        sys_config['pv_bus'],
        pv_high_cloud_to_save.copy(),
        load_active,
        penetration_ratio=0.3,
    )

    print('[5/6] Computing noise_sigma...')
    ...
    print(f'[6/6] Saving the five basic .npy files under gen_data/{sys_config["case_name"]}...')
    ...
    print('\nNext step: run the measurement generator:')
    print('  python ./gen_data/gen_data.py')
