...
SUITE_LOG="logs/case39_clean_suite_from_tau.log"
if [[ ! -f "$SUITE_LOG" ]]; then
  : >"$SUITE_LOG"
fi
exec >>"$SUITE_LOG" 2>&1
...
run_clean() {
  local tau="$1"
  local label="$2"
  local log_path="logs/case39_clean_${label}.log"
  : >"$log_path"
  echo "[runner] starting clean ${label} tau=${tau}"
  .venv_rocm/bin/python -u evaluation_event_trigger_clean.py \
    --tau_verify "$tau" \
    --max_total_run -1 \
    --stop_ddd_alarm_at -1 >>"$log_path" 2>&1
  echo "[runner] finished clean ${label} tau=${tau}"
}

run_clean "-1.0" "baseline"
run_clean "$TAU_MAIN" "main"
run_clean "$TAU_STRICT" "strict"
