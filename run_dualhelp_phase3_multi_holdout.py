...
from dual_help_phase3_core import (
    aggregate_multi_holdout_results,
    evaluate_on_holdout_bank,
    prepare_train_val_context,
    save_json,
    tune_dual_help_admission_plus_phase3_dispatch,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run dual-updated help-admission + phase3-dispatch on a frozen multi-holdout manifest."
    )
    parser.add_argument("--workdir", type=str, required=True)
    parser.add_argument("--manifest", type=str, required=True, help="Path to phase3 multi-holdout manifest.json")
    parser.add_argument("--lagadm_aggregate", type=str, default="metric/case14/lagadm_phase3_multi_holdout/aggregate_summary.json")
    parser.add_argument("--output", type=str, default="metric/case14/dualhelp_phase3_multi_holdout/aggregate_summary.json")
...
def main() -> int:
    ...
    lagadm_aggregate = None
    lagadm_path = workdir / ns.lagadm_aggregate if not os.path.isabs(ns.lagadm_aggregate) else Path(ns.lagadm_aggregate)
    if lagadm_path.exists():
        lagadm_aggregate = _load_json(lagadm_path)

    ctx = prepare_train_val_context(args)
    tuned_by_slot = tune_dual_help_admission_plus_phase3_dispatch(args, ctx)

    per_holdout: List[Dict[str, object]] = []
    for idx, holdout in enumerate(manifest["holdouts"]):
        tag = str(holdout["tag"])
        test_bank = workdir / holdout["test_bank"]
        dualhelp_eval = evaluate_on_holdout_bank(
        ...
        for slot_key, slot_res in dualhelp_eval["slot_budget_results"].items():
            ...
            row = {
                "dual_help_phase3_dispatch": slot_res["dual_help_phase3_dispatch"],
            ...
            if lagadm_aggregate is not None:
                # Align by holdout index; lagadm aggregate uses same manifest ordering.
                lag_row = lagadm_aggregate["per_holdout_results"][idx]["slot_budget_results"][slot_key]["lag_adm_phase3_dispatch"]
                row["lag_adm_phase3_dispatch"] = {
                    "weighted_attack_recall_no_backend_fail": float(lag_row["weighted_attack_recall_no_backend_fail"]),
                    "unnecessary_mtd_count": int(lag_row["unnecessary_mtd_count"]),
                    "queue_delay_p95": float(lag_row["queue_delay_p95"]),
                    "average_service_cost_per_step": float(lag_row["average_service_cost_per_step"]),
                    "pred_expected_consequence_served_ratio": float(lag_row["pred_expected_consequence_served_ratio"]),
                }
            rec["slot_budget_results"][slot_key] = row
        per_holdout.append(rec)

    aggregate = {
        "method": "dual_updated_help_admission_plus_phase3_dispatch",
        "manifest": manifest,
        "config": {
            "decision_step_group": int(args.decision_step_group),
    ...
    print(f"Saved dual-help + phase3-dispatch aggregate summary: {output_path}")
