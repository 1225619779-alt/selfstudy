...
from figure_utils import add_panel_label, apply_ieee_style, bar_hatch, budget_badge, dedupe_ablation_methods, find_repo_root, load_csv, output_dir, pretty_method, require_columns, save_figure, style_bar

METHOD_ORDER = ["phase3_reference", "oracle_fused_ec", "oracle_protected_ec"]
METRICS = [
...
def build_figure(repo_root: Path) -> None:
    ...
    df = load_csv(repo_root, "case14_ablation.csv")
    require_columns(df, ["slot_budget", "method", "recall_mean", "unnecessary_mean", "cost_mean"], "case14_ablation.csv")
    df = dedupe_ablation_methods(df)
    df = df[df["method"].isin(METHOD_ORDER)].copy()

    fig, axes = plt.subplots(2, 3, figsize=(6.8, 4.1), sharex="col")
    ...
    for row, slot in enumerate([1, 2]):
        ...
        for col, (metric, title) in enumerate(METRICS):
            ...
            ax.set_xticks(x)
            if row == 1:
                ax.set_xticklabels([pretty_method(m) for m in METHOD_ORDER])
            else:
                ax.set_xticklabels([])
            ...
            if row == 0 and col == 0:
                add_panel_label(ax, "(c)", x=-0.17, y=1.07)

    fig.subplots_adjust(left=0.09, right=0.99, top=0.94, bottom=0.12, wspace=0.30, hspace=0.09)
    save_figure(fig, out_dir, "fig03_case14_ablation")
    plt.close(fig)
