...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Validation-only regime selection for phase-3 scheduler.")
...
def _build_namespace(base: argparse.Namespace, *, decision_step_group: int, busy_time_quantile: float, use_cost_budget: bool, cost_budget_quantile: float) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        train_bank=base.train_bank,
        tune_bank=base.val_bank,
        eval_bank=base.val_bank,
        n_bins=base.n_bins,
...
def main() -> None:
    ...
    print(f"Saved validation-only regime ranking json: {args.output}")
    print("\n=== Validation-only top regimes by slot budget ===")
