...
def simulate_policy(jobs: Sequence[AlarmJob], *, total_steps: int, cfg: SimulationConfig) -> Dict[str, object]:
    ...
    for step in range(int(total_steps)):
        ...
        if available_servers > 0 and queue:
            ...
            scored.sort(key=lambda x: ((x[0] >= float(cfg.admission_score_threshold)) if getattr(cfg, "p02_fifo_order", False) else x[0], -x[1].enqueue_step, -x[1].job.job_id), reverse=True)
