...
class SimulationConfig:
    policy_name: str
    slot_budget: int = 1                      # number of backend servers
    max_wait_steps: int = 10
    threshold: Optional[float] = None
    adaptive_gain: float = 0.0               # only for adaptive threshold baseline
    v_weight: float = 1.0
    age_bonus: float = 0.0
...
def build_jobs_from_arrays(
    ...
    *,
    ...
    required_pred = [p_hat, tau_hat, cost_hat, fail_hat, value_proxy]
...
def _dynamic_threshold(step: int, queue_len: int, active_servers: Sequence[_ActiveServer], cost_spent_window: float, cfg: SimulationConfig) -> float:
    ...
    pressure = (
        _queue_pressure(queue_len, cfg)
        + _server_pressure(active_servers, cfg)
        + _cost_pressure(cost_spent_window, cfg)
    )
...
def _admission_accept(
    ...
    *,
    ...
    """Return (accept, threshold_used_if_any)."""
...
def _policy_score(
    ...
    *,
    ...
    if cfg.policy_name in {"threshold_verify_fifo", "threshold_ddd_fifo", "adaptive_threshold_verify_fifo"}:
        # All threshold baselines use FIFO inside the admitted queue.
        return -(item.enqueue_step + job.job_id * 1e-6)
...
def simulate_policy(
    ...
    *,
    ...
    dropped_jobs_budget_blocked: List[int] = []  # informative only, not removed from queue
    ...
    for step in range(int(total_steps)):
        ...
        # Arrivals first. Admission may depend on current pressure.
        ...
        if available_servers > 0 and queue:
            ...
            scored.sort(
                key=lambda x: (x[0], -x[1].enqueue_step, -x[1].job.job_id),
                reverse=True,
            )
            ...
            for score, item in scored:
                ...
                if cfg.policy_name == "proposed_vq_hard" and score <= 0.0:
                    residual_queue.append(item)
                    continue
            ...
            # Add any items that were never iterated into residual queue.
        ...
        for item in selected_items:
            ...
            active_servers.append(
                _ActiveServer(
                    job_id=job.job_id,
                    busy_until_step=int(step + max(int(job.actual_busy_steps), 1)),
                )
            )
    ...
    result = {
        ...
        "summary": {
            ...
            "server_utilization": float(occupied_server_steps / max(total_steps * max(cfg.slot_budget, 1), 1)),
            "mean_queue_len": float(np.mean(queue_len_trace)) if queue_len_trace else 0.0,
            "max_queue_len": int(max(queue_len_trace)) if queue_len_trace else 0,
            "mean_active_servers": float(np.mean(active_server_trace)) if active_server_trace else 0.0,
            "final_queue_len": int(len(queue)),
            "mean_threshold_used": float(np.mean(threshold_trace)) if threshold_trace else (
                float(cfg.threshold) if cfg.threshold is not None else np.nan
            ),
        },
